...
class Dataset(data.Dataset):
    def __init__(self, gt_file, config, mask_file=None):
        gt_file = '/scratch/sg7729/hpml_project/celeba/img_align_celeba/'
        self.gt_file = '/scratch/sg7729/hpml_project/celeba/img_align_celeba/'
        self.gt_image_files = self.load_file_list(gt_file)
        print('images', len(self.gt_image_files))
    ...
    def load_mask(self, index, img):
        _, w, h = img.shape
        image_shape = [w, h]
        self.mask_type = "random_bbox"
        if self.mask_type == "random_bbox":
            bboxs = []
            for i in range(1):
                bbox = random_bbox('dummy', image_shape)
                bboxs.append(bbox)
            mask = bbox2mask(bboxs, image_shape, 'dummy')
            return torch.from_numpy(mask)

        elif self.mask_type == "random_free_form":
            mask = random_ff_mask('dummy', image_shape)
            return torch.from_numpy(mask)
